...
<%-- 					<c:if test="${dto.dorm_ex_fl == 1}">
					<tr>					
						<td colspan="7" align="center">외박 승인 현황이 없습니다.</td>
					</tr>
					</c:if> --%>						
					</c:if>													
				...
				<tr>
					<th align="center"><c:if test="${cnt>0}">
							...
							<c:if test="${startPage > pageBlock}">
								<a href="dt_admin_overnight_sel">◀◀</a>
								<a href="dt_admin_overnight_sel?pageNum=${startPage-pageBlock}">&nbsp;◀</a>
							</c:if>
							...
							<c:forEach var="i" begin="${startPage}" end="${endPage}">
								...
								<c:if test="${i!=currentPage}">
									<a href="dt_admin_overnight_sel?pageNum=${i}">[${i}]</a>
								</c:if>
							</c:forEach>
							...
							<c:if test="${pageCount > endPage}">
								<a href="dt_admin_overnight_sel?pageNum=${startPage+pageBlock}">▶&nbsp;</a>
								<a href="dt_admin_overnight_sel?pageNum=${pageCount}">▶▶</a>
							</c:if>
						</c:if></th>
				</tr>
			</table>
