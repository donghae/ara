				...
				<div id="result">
					<div>
						...
						<div style="width: 100%; z-index: 1;">
							<h3 style="float: left">수강 신청 현황</h3>
							<input style="float: right; left: 400px;" class="btn_mint"
								type="button" value="시간표 보기" onclick="timeTable();">
						</div>
					</div>
								...
								<c:if test="${cnt_reg>0}">
									<c:forEach var="dto" items="${dtos_reg}">
										<tr>
											...
											<td style="text-align: center"><input class="btn_mint"
												name="regDel" type="button" value="취소" id="${dto.lec_no}">
											</td>
										</tr>
									</c:forEach>
								</c:if>
						...
						<table class="st_mint"
							...
							<c:if test="${cnt_wish>0}">
								<c:forEach var="dto" items="${dtos_wish}">
									<tr>
										<td style="text-align: center"><input class="btn_mint"
											name="regPlus" type="button" value="신청" id="${dto.lec_no}">
										</td>
										<td style="text-align: center">${dto.lec_no}</td>
										<td style="text-align: center">${dto.lec_name}</td>
										<td style="text-align: center">${dto.lec_point}</td>
