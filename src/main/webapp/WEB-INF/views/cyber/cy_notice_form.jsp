				...
				<div class="cen_middle">
					<div align="center" style="min-height: 50%;">
						...
						<div class="left_div" style="width: 80%; min-height: 40%;">
							<form action="cy_notice_update_form" method="post">
								<input type="hidden" name="num" value="${vo.cy_n_no}">
								<table class="st_mint">
									<tr>
										<th style="width: 15%">제목</th>
										<td><c:if
												test="${fn:substring(sessionScope.id,0,1) == 0 || fn:substring(sessionScope.id,0,1) == 1}">
												<input type="text" name="title" required="required" value="${vo.cy_n_title}">
							...
							${vo.cy_n_title}
									...
									<tr>
										...
										<td><a style="color: black;"
											href="cy_download?fileName=${vo.cy_n_file}">${vo.cy_n_file}</a>
										</td>
									</tr>
									<tr>
										<th>내용</th>
										<td><c:if
												...
												<textarea name="content" class="input" rows="10" cols="40"
													name="content"
													style="width: 80%; resize: none; height: 302px;">${vo.cy_n_content}</textarea>
											</c:if> <c:if test="${fn:substring(sessionScope.id,0,1) == 2}">
												<textarea name="content" class="input" rows="10" cols="40"
													name="content"
													style="width: 80%; resize: none; height: 302px; border: none; outline: none;"
													readonly="readonly" required="required">${vo.cy_n_content}</textarea>
											</c:if></td>
									</tr>
									<tr>
										<th colspan="2"><input class="btn_mint" type="button"
											value="목록"
											onclick="window.location='cy_main?pageNum=${pageNum}'">
											<c:if
												test="${fn:substring(sessionScope.id,0,1) == 0 || fn:substring(sessionScope.id,0,1) == 1}">
												<input class="btn_mint" type="submit" value="글 수정">
												<input class="btn_mint" type="button" value="글 삭제"
													onclick="window.location='cy_notice_delete_form?num=${vo.cy_n_no}'">
											</c:if></th>
									</tr>
								</table>
							</form>
